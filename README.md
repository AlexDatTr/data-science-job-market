...
1. Noun Phrase: optional determinate, any number of adjectives, a noun, noun plural, proper noun with additionals following noun
...
So in order to extract the skills from the job description, I will use the following pipeline:
1. Tokenize the job description
2. Clean the tokenized job description with lemmatization and remove stop words
3. Add Part of Speech tag to each word of the job description
4. Create 4 grammar patterns associate with 4 grammar structure above and extract the phrase that match the pattern
5. Add the extracted phrase to a list of skills and remove duplicate and export the list to a csv file for label and training purpose.

As a result, I have a list of thousands of noun phrases that are extracted from the job description. The list is then manually label to be used for training a model to extract skills.
